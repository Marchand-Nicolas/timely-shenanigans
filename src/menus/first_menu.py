        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYUP:
                if event.key == 13: # cas pour la touche entrer 
                    ...
                    text = text[:-1] 
                elif event.key == 8 and text == '': # cas pour le backspac quand il n'y a pas de psedo
                    pass
                ...
                if not text_rect.collidepoint(x, y): # si le joueur clique hors du rectengle d'entrer de texte
                    return text
        pygame.draw.rect(screen.get_pygame_screen(), (255, 255, 255), text_rect)
...
def menu_choose_game(screen):
    ...
    rect_enter_code_to_join = pygame.Rect(screen.get_width()//2 - 150, screen.get_height()//3 + 125, 300, 50)
    rect_code = pygame.Rect(screen.get_width()//2 - 150, screen.get_height()//3 + 175, 300, 50)
    text_creat_world = arial24.render("creat a new world",True,pygame.Color(0,0,0))
    text_join_world = arial24.render("join a world",True,pygame.Color(0,0,0))
    ...
    while running:
        ...
        screen.get_pygame_screen().blit(text_creat_world, (screen.get_width() // 2 - 150, screen.get_height() // 3 - 50))
        screen.get_pygame_screen().blit(text_join_world, (screen.get_width() // 2 - 150, screen.get_height() // 3 + 50))
        screen.get_pygame_screen().blit(text_enter_code, (screen.get_width() // 2 - 150, screen.get_height() // 3 + 125))
        if botton_join_clic == True:
            pygame.draw.rect(screen.get_pygame_screen(), (255, 255, 255), rect_enter_code_to_join)
            pygame.draw.rect(screen.get_pygame_screen(), (255, 255, 255), rect_code)
            screen.get_pygame_screen().blit(text_enter_code, (screen.get_width() // 2 - 150, screen.get_height() // 3 + 125))
            screen.get_pygame_screen().blit(code_text, (screen.get_width() // 2 - 150, screen.get_height() // 3 + 175))
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
            if event.type == pygame.MOUSEBUTTONDOWN: 
                ...
                elif (rect_enter_code_to_join.collidepoint(x, y) or rect_code.collidepoint(x, y) or rect_join_world.collidepoint(x, y)) and botton_join_clic == True:
                    code = enter_text(rect_code, screen, (screen.get_width() // 2 - 150, screen.get_height() // 3 + 175))
...
def show_first_menu(screen):
    ...
    text_enter_name_rect =  pygame.Rect(screen.get_width()//2 - 150, screen.get_height()//2 + 100 , 300, 50)
    enter_name_rect = pygame.Rect(screen.get_width()//2 - 150, screen.get_height()//2 + 150 , 300, 50)
    ...
    while running:
        ...
        pygame.draw.rect(screen.get_pygame_screen(), (255, 255, 255), text_enter_name_rect)  
        ...
        screen.get_pygame_screen().blit(text_enter_name, (screen.get_width()//2 - 150, screen.get_height()//2 + 100))
        screen.get_pygame_screen().blit(psedo_text, (screen.get_width()//2 - 150, screen.get_height()//2 + 150))      
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: 
                x, y = pygame.mouse.get_pos() 
                if enter_name_rect.collidepoint(x, y): # detecte si la sourie est cliquer dans le rect d'entrer de nom
                    name = enter_text(enter_name_rect, screen, (screen.get_width()//2 - 150, screen.get_height()//2 + 150))
            if event.type == pygame.MOUSEBUTTONDOWN:
                x, y = pygame.mouse.get_pos() 
                if play_buton.collidepoint(x, y): # detecte si la sourie est cliquer dans le rect de debut de jeu
                    if name == 'enter your name' or name == '':
                        messagebox.showinfo("user name", "il faut entrer un nom d'utilisateur pour pouvoir jouer")
                    else:
                        undisplay(screen)
